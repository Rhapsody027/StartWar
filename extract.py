...
from typing import List, Tuple, Dict, Optional, Any, DefaultDict, Union, Set
...
def is_text_based_page(page: Any) -> bool:
...
def extract_text_with_format(page: Any, exclude_bboxes: List[Tuple[float, float, float, float]]) -> Optional[List[str]]:
    """提取文字段落，不添加標題標記"""
    try:
        md_content: List[str] = []
        words = page.extract_words()
...
def is_narrative_table(df: pd.DataFrame) -> bool:
    """檢查表格是否為叙述性文字而非數據表格"""
...
def extract_table_with_camelot(page: Any) -> Tuple[List[str], List[Tuple[float, float, float, float]], List[str]]:
...
def extract_table_with_ppsstructure(image: np.ndarray) -> Tuple[List[str], List[Union[Tuple[float, float, float, float], Tuple[float, float, float, float]]]]:
    """使用 PPStructure 提取表格（Fallback）"""
    try:
        layout = table_engine(image)
        md_tables: List[str] = []
        bboxes: List[Union[Tuple[float, float, float, float], Tuple[float, float, float, float]]] = []
...
def pdf_to_image(pdf_path: str, page_num: int, resolution: int = 300) -> Optional[np.ndarray]:
    """將 PDF 頁面轉為圖片"""
    try:
...
def extract_table_with_ocr(image: np.ndarray) -> Tuple[Optional[List[str]], Optional[Any]]:
    """使用 PaddleOCR 重建表格"""
    try:
        ...
        lines: List[Tuple[Any, str, float]] = [(line[0], line[1][0], line[1][1]) for line in result[0]]
        if not lines:
            return None, None

        y_coords: List[float] = [line[0][0][1] for line in lines]
        x_coords: List[float] = [line[0][0][0] for line in lines]
        text_heights: List[float] = [line[0][2][1] - line[0][0][1] for line in lines]
        ...
        y_groups: DefaultDict[float, List[Tuple[float, str, Any]]] = defaultdict(list)
...
def merge_cross_page_tables(tables: List[str]) -> List[str]:
    """合併跨頁表格並清理 nan"""
    if not tables:
        return tables
    merged: List[str] = []
    current_table: Optional[pd.DataFrame] = None
    for table in tables:
        try:
...
def process_pdf(pdf_path: str, output_md_path: str) -> None:
